...
function generateServers(country) {
  const cities = CITIES_BY_CONTINENT[country.continent] || ['Ciudad Principal'];
  const count = 1 + Math.floor(Math.random() * cities.length);
  ...
  for (let i = 0; i < Math.min(count, shuffled.length); i++) {
    ...
    const ip = `${country.code.toLowerCase()}-${shuffled[i].toLowerCase().replace(/\s+/g,'-')}.vpnworld.io`;

    servers.push({
      city: shuffled[i],
      protocol: protocol,
      hostname: ip,
      ip: `${10 + Math.floor(Math.random()*200)}.${Math.floor(Math.random()*255)}.${Math.floor(Math.random()*255)}.1`,
      port: port,
      load: Math.floor(Math.random() * 80) + 5, // % load
...
// Pre-generate servers with stable random (seeded by country code)
function hashCode(s) {
  let h = 0;
